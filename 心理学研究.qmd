...
format:
  pdf:
    ...
    include-in-header: template/header_jjpsy.tex
...
```{=html}
<!-- 
原稿の準備にあたっては，日本心理学会の出している手引を参照ください。
https://psych.or.jp/manual/manual05#3_3
...
-->
```
　はじめに，ここから文章を書き始めます。心理学以降が論文の本文になります。どんどん書いていきましょう！
...
-->
```
　心理学の再現可能性はとってもまずい状況なのですが，それはちょっとおいておいて，文献の引用の仕方を説明します。まず，@Freud1956 のように，すると，bibファイル内の論文が引用されます。そして，次のように，\[\]でくくると文末の引用スタイルになります[@長谷川99]。また，文末に複数引用する場合は，こういう感じにします[@Takahashi2017; @Christie2011; @Helmholtz1925; @Morioka2018; @向田2009]。
...
```{=html}
<!-- 以下は1頁が25字*32行で800字になっているかのチェック用です。以下の数字がすべて１ページに収まっていればOKです。
...
-->
```
　Quartoは，Markdown記法が使えます。そのため以下の利点があります。(a)数式処理などのLaTeXの機能を使うことができるが，それがLaTeXよりも簡単に利用できる。(b)生データから最終的な論文まで一気につなぐことができるので，方法の再現可能性を高めることができる。(c)細かなフォーマット調整をしなくても，テンプレートがきれいな原稿を出力してくれる。
...
新たに質問紙を作成したら，ちゃんとライセンスを明記して公開してほしい・・・
